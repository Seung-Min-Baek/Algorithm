...
### 오답
오랜 시간동안 풀지 못하고 다른 분의 풀이를 참고하였다. 조건문에서 크기비교를 하는 것이 이해하기 어려운 부분이었다.
3열에 arr.append(input().split())라고 하여 계속 가로읽기가 되었다. split()은 띄어쓰기를 기준으로 문장을 나누기 때문에 리스트 arr안에 새로운 리스트가 들어간다. 예를 들면 ['A','B']와 [['A'],['B']]차이다. 그래서 그 이후에 설정한 반복문에서 다르게 적용되어 가로읽기가 되었던 것이다.
