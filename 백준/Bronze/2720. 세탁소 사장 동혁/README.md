...
동전마다 일일히 몫과 나머지를 구해주었었다. 하지만 코드가 길고 가독성이 떨어졌다. 동전들을 리스트에 모두 넣어 한번에 반복문을 해주는 방법을 발견했다. 연습이 더 필요할듯,,

### new

리스트 앞에 *을 입력하면 괄호와 쉼표를 제외한 요소들만 출력된다.
